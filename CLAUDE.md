...
| **C：ポートフォリオ品質化** | バグ修正 → README → 信頼性設定 → 冪等性 → テスト拡充 → SPA化 | タスク1完了。タスク2（README）着手中 |

**計画 A と B と C は独立している。** ユーザーの都合でいつでも切り替えてよい。
...
| # | 内容 | 参照ブランチ | 状態 |
|---|------|------------|------|
| 1 | WebSocket NPEバグ修正（`RedisWebSocketSubscriber`） | `claude/fix-websocket-npe` | ✅ 完了 |
| 2 | README 作成（アーキテクチャ図・起動手順・設計ポイント） | `claude/docs-readme` | ✅ Claude作成済み → ユーザー反映待ち |
| 3 | Kafka Producer 信頼性設定（`acks=all` / 冪等プロデューサ / 失敗ハンドリング） | 未作成 | 未着手 |
| 4 | Consumer 冪等性（重複排除：`processed_messages` テーブル） | 未作成 | 未着手 |
| 5 | テスト拡充（Kafka/Redis を含む統合テスト、Testcontainers） | 未作成 | 未着手 |
| 6 | フロント SPA 化 or 実機能追加 | 未定 | 未着手 |
