...
【Phase 2】非同期・イベント駆動  ← 現在
  🔄 Apache Kafka
     ...
     ❌ 動作確認（docker-compose up → アプリ起動 → ログ確認）
     ❌ Consumer での実処理実装（メール送信を@KafkaListener側へ移植）
...
**次の具体的な作業: `docker-compose up kafka` でブローカー起動 → アプリ起動 → 経費申請して Kafka の consume ログを確認**
