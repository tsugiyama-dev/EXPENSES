...
```
【Phase 1】今動いているものを大規模対応させる   ← 現在ここ
...
1. **実画面への組み込み** - `detail.html`（経費一覧）に `notification.js` を埋め込み、実際の承認操作時に通知が届く状態にする
2. **ユーザーID の動的取得** - Thymeleaf でログインユーザーIDをHTMLに埋め込み `notification.js` に渡す
3. **Redis Pub/Sub 対応** - 複数インスタンス時でも通知が届くようスケールアウト対応
