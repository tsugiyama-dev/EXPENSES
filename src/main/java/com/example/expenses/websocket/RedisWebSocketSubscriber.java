...
public class RedisWebSocketSubscriber {
	...
	public void onMessage(NotificationMessage message) {
		...
		log.debug("Redis subscribe: destination={}, expenseId={}", destination, message.getExpenseId());

		// クライアントには内部ルーティング情報を渡さない
		...
		if (destination.startsWith("/topic")) {
			...
			// 個人あて（/queue/notifications）→ /user/{email}/queue/notifications へ届く
